...
`ScrollPresenter` 默认使用动画处理 Avalonia 的无参数 `BringIntoView()`。将
`IsBringIntoViewAnimationEnabled` 设置为 `false`，则改为立即执行最小距离滚动。
引入 `SmoothScroll.Avalonia.Controls` 命名空间后，还可以使用以下支持动画的重载：
